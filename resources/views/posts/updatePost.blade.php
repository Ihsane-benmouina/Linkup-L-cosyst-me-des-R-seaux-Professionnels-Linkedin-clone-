...
<body class="bg-[#f8fafc] text-[#0f172a] antialiased flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-lg bg-white border border-slate-200/60 rounded-2xl p-6 shadow-[0_1px_3px_rgba(0,0,0,0.02)]">
        <div class="flex justify-between items-center mb-5">
            <h1 class="text-sm font-bold text-slate-900 uppercase tracking-wider">Modifier la publication</h1>
        ...
        <form action="{{ route('posts.updatePost', $post) }}" method="POST" class="space-y-4">
            ...
            <div>
                <label class="block text-[11px] font-bold text-slate-700 uppercase tracking-wider mb-2">Contenu du post</label>
                <textarea name="content" rows="5" required
                          class="w-full bg-slate-50/50 border border-slate-200 rounded-xl p-3 text-xs focus:outline-none focus:border-slate-900 focus:bg-white transition resize-none">{{ old('content', $post->content) }}</textarea>
                @error('content')
